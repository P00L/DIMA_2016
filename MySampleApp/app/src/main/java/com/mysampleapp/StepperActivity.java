...
import com.amazonaws.mobile.stepper.VerticalStepper;
...
public class StepperActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        fab.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                Intent intent = new Intent(getApplicationContext(), VerticalStepper.class);
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode == Activity.RESULT_OK && requestCode == NEW_ALARM && data != null) {
            if(data.hasExtra(VerticalStepper.NEW_ALARM_ADDED)
                    && data.getExtras().getBoolean(VerticalStepper.NEW_ALARM_ADDED, false)) {

                // Handling the data received from the stepper form
                dataReceived = true;
                String title = data.getExtras().getString(VerticalStepper.STATE_TITLE);
                //String description = data.getExtras().getString(NewAlarmFormActivity.STATE_DESCRIPTION);
                int hour = data.getExtras().getInt(VerticalStepper.STATE_TIME_HOUR);
                int minutes = data.getExtras().getInt(VerticalStepper.STATE_TIME_MINUTES);
                ...
                disclaimer.setVisibility(View.VISIBLE);
                Snackbar.make(fab, getString(R.string.new_stuff_added), Snackbar.LENGTH_LONG).show();
            }
        }
    }
}